...
export default function TriagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [borradorEBITDA, setBorradorEBITDA] = useState<Record<string, unknown> | null>(null)
  ...
  async function readFiles(fileList: FileList) {
    ...
    for (const f of Array.from(fileList)) {
      ...
      const mime = ext==="pdf"?"application/pdf":ext==="png"?"image/png":(ext==="jpg"||ext==="jpeg")?"image/jpeg":"text/plain"
  ...
  async function applySelected() {
    ...
    const total = Object.values(seleccion).reduce((s, arr) => s + arr.length, 0)
    if (!total) { showToast("Tildá al menos una propuesta", false); setApplying(false); return }
    try {
      const res = await fetch("/api/triage", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json()
      showToast(`✅ ${data.aplicados} cambios aplicados${data.errores?.length ? ` · ${data.errores.length} errores` : ""}`, data.ok)
      if (data.errores?.length) console.error("Errores triage:", data.errores)

      // Actualizar borrador EBITDA si aplica
      if (data.debeEBITDA) {
        ...
        try {
          ...
          if (ebData.ok) { setBorradorEBITDA(ebData.borrador); showToast("📊 Borrador EBITDA actualizado") }
        } catch {}
        setGenerandoEBITDA(false)
      }

      setResultado(null); setFiles([])
    } catch { showToast("Error de conexión", false) }
    setApplying(false)
  }

  const fmtUSD = (n: number) => `${n<0?"-":""}USD ${Math.abs(n).toLocaleString("es-AR")}`

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-xl font-bold text-gray-900 mb-1">Triage de Documentos</h1>
      <p className="text-sm text-gray-500 mb-5">La IA analiza y propone — vos aprobás antes de aplicar</p>

      {/* Upload zone */}
      <div className="card border-dashed border-2 border-gray-300 hover:border-navy-DEFAULT transition-colors cursor-pointer mb-4"
        onClick={() => inputRef.current?.click()}
        onDrop={e => { e.preventDefault(); readFiles(e.dataTransfer.files) }}
        onDragOver={e => e.preventDefault()}>
        <div className="text-center py-6">
          <Upload size={28} className="mx-auto text-gray-400 mb-2"/>
          <p className="text-sm font-medium text-gray-700">Arrastrá archivos o hacé clic para seleccionar</p>
          <p className="text-xs text-gray-500 mt-1">PDF (multimodal real) · Imágenes · XLSX · DOCX · múltiples archivos</p>
        </div>
        <input ref={inputRef} type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.xlsx,.csv,.docx,.txt"
          className="hidden" onChange={e => e.target.files && readFiles(e.target.files)}/>
      </div>
        ...
        <div className="space-y-4">
          ...
          {(resultado.items?.length ?? 0) > 0 && (
            <div className="card">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-sm">Ítems del Tracker ({resultado.items!.length})</h3>
                <span className="text-xs text-gray-500">— hechos literales del documento</span>
                <button onClick={() => setSelItems(selItems.size<resultado.items!.length?new Set(resultado.items!.map((_,i)=>i)):new Set())} className="ml-auto text-xs text-gray-500 hover:text-gray-700">
                  {selItems.size<resultado.items!.length?"Tildar todos":"Destildar todos"}
                </button>
              </div>
              {resultado.items!.map((it, i) => (
                <div key={i} className={`flex gap-3 p-3 rounded-lg mb-2 border cursor-pointer ${selItems.has(i)?"border-blue-200 bg-blue-50":"border-gray-100 bg-gray-50"}`}
                  ...
                  <div className="flex-1">
                    ...
                    {it.alertas && <p className="text-xs text-amber-700 mt-0.5"><b>⚠️ Alertas:</b> {it.alertas}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {(resultado.riesgos?.length ?? 0) > 0 && (
            <div className="card border-amber-200">
              ...
              <p className="text-xs text-amber-700 mb-3">Son estimaciones — destildados por default. Revisalos antes de aplicar.</p>
          ...
          {(resultado.actualizaciones_hojas?.length ?? 0) > 0 && (
            <div className="card">
              <div className="flex items-center gap-2 mb-3">
                <FileText size={14} className="text-navy-DEFAULT"/>
              ...
              {resultado.actualizaciones_hojas!.map((h, i) => (
                <div key={i} className={`flex gap-3 p-3 rounded-lg mb-2 border cursor-pointer ${selHojas.has(i)?"border-navy-50 bg-blue-50":"border-gray-100 bg-gray-50"}`}
                  ...
                  <div className="flex-1">
                    <div className="text-xs font-bold text-navy-DEFAULT">{iconoHoja(h.hoja)} {h.hoja}{h.clave?" — "+h.clave:""}</div>
      ...
      {(generandoEBITDA || borradorEBITDA) && (
        <div className="card mt-5 border-amber-200 bg-amber-50">
          ...
          {generandoEBITDA ? <p className="text-sm text-amber-700">Generando borrador incremental...</p> : borradorEBITDA ? (
            <div className="space-y-1.5 text-xs">
              <div><b>Piezas disponibles:</b> ítems {(borradorEBITDA.piezas_disponibles as number[])?.join(", ") || "ninguna"}</div>
              ...
              {borradorEBITDA.ebitda_normalizado_tentativo!=null && (
                <div className="font-bold text-amber-900 pt-2 border-t border-amber-200">
                  EBITDA NORMALIZADO TENTATIVO: USD {(borradorEBITDA.ebitda_normalizado_tentativo as number).toLocaleString("es-AR")}
                </div>
              )}
              <p className="text-gray-500 italic">{borradorEBITDA.advertencias as string}</p>
              <p className="text-gray-400 text-xs">Borrador guardado en las notas del ítem 48. El ítem sigue PENDIENTE.</p>
            </div>
          ) : null}
        </div>
      )}
