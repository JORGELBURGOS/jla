    ...
    try {
      switch (a.tipo) {
        ...
        case 'actualizar_item': {
          ...
          await db.from('dd_case_requirements').update({
            [col]: valorFinal,
        ...
        case 'actualizar_riesgo': {
          ...
          await db.from('dd_case_risks').update(updates).eq('id', match.id)
        ...
        case 'actualizar_hoja': {
          ...
          if (hoja === 'Síntesis Ambiental' || hoja.includes('Ambiental')) {
            ...
            await db.from('dd_case_environmental').update(upd).eq('id', row.id)
            ...
            await db.from('dd_case_validation').update(upd).eq('id', row.id)

          } else if (hoja === 'Análisis Fiscal' || hoja === 'Valuación') {
            // Solo notas al pie — append a dd_audit_log
